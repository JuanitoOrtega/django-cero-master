  ...
  def get_graph_sales_products_year_moth(self):
    ...
    try:
      for p in Product.objects.all():
        total = DetailSale.objects.filter(sale__date_joined__year=year, sale__date_joined__month=month, product_id=p.id).aggregate(r=Coalesce(Sum('quantity'), 0, output_field=FloatField())).get('r')
        if total > 0:
          data.append({
            'name': p.product_name,
            'y': float(total),
